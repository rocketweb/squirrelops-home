...
def generate_host_tls_identity(
    ...
    mdns_hostname = f"{label}.local"
    dns_names = [canonical]
    if mdns_hostname != canonical:
        dns_names.append(mdns_hostname)
    # X.509 Common Name values are limited to 64 characters. A valid
    # 63-character DNS label plus either supported suffix can exceed that
    # legacy field, while remaining valid in Subject Alternative Name.
    common_name = canonical if len(canonical) <= 64 else label
