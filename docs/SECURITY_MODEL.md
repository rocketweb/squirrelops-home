...
allowlist, never the service `PATH`. Dynamic Python plugins are accepted only
from a non-writable directory and regular files owned by an explicitly trusted
UID. Packaged operation does not currently load third-party plugins.
